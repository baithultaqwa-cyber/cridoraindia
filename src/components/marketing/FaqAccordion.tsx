    ...
    <ul className="mx-auto max-w-3xl space-y-3">
      {items.map((item, i) => {
        ...
        return (
          <li
            key={item.q}
            className="overflow-hidden rounded-2xl border border-silk/15 bg-navy-silk/50 backdrop-blur-md transition-colors duration-300 hover:border-cridora-gold/20"
          >
            <button
              type="button"
              id={`${id}-btn`}
